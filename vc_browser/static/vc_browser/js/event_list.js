...
__init__ : function(_el_dom_id, _ed_dom_id, initial_list, options) {
    $this = this;
    ...
    this.number_items_showing = parseInt(options.default_events_per_page);
    ...
    event_list_display_items_str = "";
    for ( option in options.events_per_page_options ) {
        event_list_display_items_str += "<li" + ((parseInt(options.events_per_page_options[option]) == this.number_items_showing) ? " class=\"selected\"" : "") + ">" + options.events_per_page_options[option] + "</li>"
        //console.debug(options.events_per_page_options[option])
    }
    ...
    this._el_controls_dom_jq.append("<div id=\"event_list_selection\">\
                                    ...
                                    <div id=\"event_list_display_items\" class=\"ss_popup\">\
                                        <ul>" + event_list_display_items_str + "</ul>\
                                    </div>");
...
pageClicked : function(event) {
    ...
    if ( selected_page != parseInt($this.current_page) ) {
        //console.debug([selected_page,  $this.current_page]);
        ...
        $this.getNewEventList($this.number_items_showing, current_sort_column, current_sort_direction, $this.current_page);
    }
...
eventListDisplayItemCallback : function(params) {
    ...
    $this.getNewEventList(params.selection, current_sort_column, current_sort_direction, $this.current_page);
...
getNewEventList : function( items_to_display, sort_column, sort_direction, page ) {
    // create an overlay if it takes a while to get the list
    ...
    $.getJSON("get_event_list.json", {items_to_display:items_to_display, sort_column:sort_column, sort_direction:sort_direction, page_number:page-1},function(data) {
        $this.list_dat = data;
        $this.setPageWidth();
        $this.buildList();
...
buildList : function(event) {
    ...
    for ( row in $this.list_dat.Records ) {
        ...
        content_str += "<tr class=\"event_list_row " + even_odd + "\">\
                            ...
                            <td class=\"event_list_cell evyear\" style=\"width:"+ $this.column_info.evyear.column_width +"%\">" + $this.list_dat.Records[row].evyear.toFixed(3) + "</td>\
                            <td class=\"event_list_cell evmag\" style=\"width:"+ $this.column_info.evmag.column_width +"%\">" + $this.list_dat.Records[row].evmag.toFixed(3) + "</td>\
                            ...
                            <td class=\"event_list_cell evaveslip\" style=\"width:"+ $this.column_info.evaveslip.column_width +"%\">" + $this.list_dat.Records[row].evaveslip.toFixed(3) + "</td>\
...
hideEventList : function($this, event_num) {
    ...
    $this._el_dom_jq.animate({
        ...
        $this.loadEventDetail(event_num);
    });
    
},

loadEventDetail : function( event_num ) {
    // create an overlay if it takes a while to get the list
    $this.activity_overlay = $("<div class=\"activity_overlay\"><div class=\"activity_overlay_message\">Please wait.</div></div>").appendTo("#uber");
    $this.activity_overlay.css({"position":"absolute", "display":"none"});
    $this.activity_overlay.offset(
    ...
    //console.debug(event_num);
    var csrfToken = $('input[name="csrfmiddlewaretoken"]').val();
    
    
    $.get("event_detail.html", {event_num:event_num}, function(data) {
        $this.activity_overlay.remove();
        $this._ed_dom_jq.html(data);
...
detailLoadComplete : function($this) {
    ...
    $this._ed_dom_jq.prepend("<div id=\"back\"></div>");
    ...
    top_items_height = Math.max($("#event_info").outerHeight(true) + $("#event_time_series").outerHeight(true) + back_button.outerHeight(true), $("#event_fault_map").outerHeight(true) + back_button.outerHeight(true));
    
    $("#event_fault_map").css({"position":"absolute", "top":back_button.outerHeight(true), "left":$("#event_time_series").outerWidth() + parseFloat($("#event_fault_map").css("margin-left").split("px")[0])});
    
    $("#event_rupture_map").css({"position":"absolute", "top":top_items_height});
...
eventDetailClicked : function(event) {
    ...
    //console.debug($(event.delegateTarget).parent("tr").children(".evid").text());
    
    event_num = parseInt($(event.delegateTarget).parent("tr").children(".evid").text());
...
listHeaderClicked : function(event) {
    ...
    $this.getNewEventList($this.number_items_showing, selected_column, sort_direction, $this.current_page);
